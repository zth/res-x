module CacheControl = {
  ...
  type revalidation =
    ...
    | @as("stale-while-revalidate") StaleWhileRevalidate(time)
    /** This is for the directive "stale-if-error". Uses the stale response if revalidation fails. */
    | @as("stale-if-error") StaleIfError(time)
  ...
  type extensions =
    ...
    | @as("wait-while-invalidate") WaitWhileInvalidate(time)
