...
for (const mode of ['baseline', 'optimized']) {
  const instance = createResxBunPlugin();
  try {
    const build = await Bun.build({entrypoints, outdir: `.resx/tests/${mode}`, naming: '[name].js', target: 'bun', format: 'esm', plugins: mode === 'optimized' ? [instance.plugin] : []});
    ...
    if (mode === 'optimized') assert(instance.transform.stats.templates > 0);
