...
const plugin = baseline ? null : resxBunPlugin({cacheDir: join(root, '.resx/cache/oxc')});
...
const result = await Bun.build({entrypoints: [entry], outdir, target: 'bun', format: 'esm',
  ...
  naming: 'app.mjs', sourcemap: 'external', plugins: plugin ? [plugin] : []});
...
const report = {mode: baseline ? 'baseline' : 'oxc', entry, outdir, stockCompileMs,
  bundleMs: performance.now() - bundleStart, totalMs: performance.now() - start, ...plugin?.stats};
